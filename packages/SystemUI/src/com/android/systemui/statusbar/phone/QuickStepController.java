...
public class QuickStepController implements GestureHelper {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                if (mNavigationBarView.isDt2s() && isDoubleTapPending) {
                    ...
                    if (isDoubleTapReally) SyberiaUtils.switchScreenOff(mContext);
                } else {
            ...
            case MotionEvent.ACTION_UP:
               if (mNavigationBarView.isFullGestureMode()) {
                    ...
                    if (mBackActionScheduled) {
                        endQuickScrub(true /* animate */);
                        SyberiaUtils.sendKeycode(KeyEvent.KEYCODE_BACK, mHandler);
                    } else {
                        endQuickScrub(true /* animate */);
                    }
                    break;
    ...
    private Runnable mDoubleTapCancelTimeout = new Runnable() {
        ...
        public void run() {
            ...
            mHandler.removeCallbacksAndMessages(null);
            SyberiaUtils.sendKeycode(KeyEvent.KEYCODE_HOME, mHandler);
        }
    };
    ...
    private void moveKbCursor(boolean right, boolean firstTrigger) {
        ...
        SyberiaUtils.moveKbCursor(KeyEvent.ACTION_UP, right);
        SyberiaUtils.moveKbCursor(KeyEvent.ACTION_DOWN, right);
