...
public class ThemeAccentUtils {
    ...
    private static final String[] SYBERIA_THEMES = {
        "com.android.system.theme.syberia", // 0
        "com.android.settings.theme.syberia", // 1
        "com.android.systemui.theme.syberia", // 2
    };
    ...
    // Check for the Syberia system theme
    public static boolean isUsingSyberiaTheme(IOverlayManager om, int userId) {
        OverlayInfo themeInfo = null;
        try {
            themeInfo = om.getOverlayInfo(SYBERIA_THEMES[0],
                    userId);
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    ...
    public static void setLightSyberiaTheme(IOverlayManager om, int userId, boolean useSyberiaTheme) {
        for (String theme : SYBERIA_THEMES) {
                try {
                    om.setEnabled(theme,
                        useSyberiaTheme, userId);
                    unfuckBlackWhiteAccent(om, userId);
                } catch (RemoteException e) {
                    Log.w(TAG, "Can't change theme", e);
                }
