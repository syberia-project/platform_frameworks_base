...
public final class SQLiteSession {
    ...
    public void beginTransaction(int transactionMode,
        ...
        beginTransaction(transactionMode, transactionListener, connectionFlags, null,
                cancellationSignal);
    }
        ...
        try {
            ...
            if (mTransactionStack == null) {
                ...
                switch (transactionMode) {
                    case TRANSACTION_MODE_IMMEDIATE:
                        mConnection.execute(authorizer, "BEGIN IMMEDIATE;", null,
                                cancellationSignal); // might throw
                        break;
                    case TRANSACTION_MODE_EXCLUSIVE:
                        mConnection.execute(authorizer, "BEGIN EXCLUSIVE;", null,
                                cancellationSignal); // might throw
                        break;
                    default:
                        mConnection.execute(authorizer, "BEGIN;", null,
                                cancellationSignal); // might throw
                        break;
                }
            }
            ...
            if (transactionListener != null) {
                try {
                    ...
                    if (mTransactionStack == null) {
                        mConnection.execute(authorizer, "ROLLBACK;", null,
                                cancellationSignal); // might throw
                    }
                    throw ex;
                }
     ...
     * @see #yieldTransaction
     */
    public void endTransaction(@Nullable SQLiteAuthorizer authorizer,
            @Nullable CancellationSignal cancellationSignal) {
        throwIfNoTransaction();
        assert mConnection != null;

        endTransactionUnchecked(authorizer, cancellationSignal, false);
    }

    private void endTransactionUnchecked(@Nullable SQLiteAuthorizer authorizer,
            @Nullable CancellationSignal cancellationSignal, boolean yielding) {
        if (cancellationSignal != null) {
            cancellationSignal.throwIfCanceled();
        }
        ...
        if (mTransactionStack != null) {
            ...
            try {
                if (successful) {
                    mConnection.execute(authorizer, "COMMIT;", null,
                            cancellationSignal); // might throw
                } else {
                    mConnection.execute(authorizer, "ROLLBACK;", null,
                            cancellationSignal); // might throw
    ...
    private boolean yieldTransactionUnchecked(long sleepAfterYieldDelayMillis,
        ...
        endTransactionUnchecked(null, cancellationSignal, true); // might throw
    ...
    public void execute(@Nullable SQLiteAuthorizer authorizer,
        ...
        if (executeSpecial(authorizer, sql, bindArgs, connectionFlags, cancellationSignal)) {
            return;
        }
     ...
     * @throws OperationCanceledException if the operation was canceled.
     */
    private boolean executeSpecial(@Nullable SQLiteAuthorizer authorizer,
            @NonNull String sql, @Nullable Object[] bindArgs, int connectionFlags,
            @Nullable CancellationSignal cancellationSignal) {
        if (cancellationSignal != null) {
            cancellationSignal.throwIfCanceled();
        ...
        switch (type) {
            ...
            case DatabaseUtils.STATEMENT_COMMIT:
                ...
                endTransaction(authorizer, cancellationSignal);
                return true;
